...
author: "Erin Chille"
date: "DRAFT: Last updated 2020/11/2"
...
```{r}
sample.info <- read.csv(file="2b-Epi-MZT-Enzyme-Expression/Input/RNAseq_data.csv", header=T, sep=",") #load sample info
```

Load gene count Information
```{r}
counts <- as.data.frame(read.csv("2b-Epi-MZT-Enzyme-Expression/Input/gene_abund_TPM.csv", row.names="gene_id"), colClasses = double) #Load normalized expression matrix from stringtie
```
...
Fig.5A
ggsave("2b-Epi-MZT-Enzyme-Expression/Output/Fig5-Mcap-Dev-Epi-Expression.pdf", Fig.5A, width=21, height=15)
```
...
Compile plots into single figure and save
```{r}
Fig.5 <- cowplot::plot_grid(Mcap.SMG, Mcap.KAISO, Mcap.TBXT, Mcap.SOX2, Mcap.NANOG, Mcap.WNT8, align = "v", ncol = 3)
Fig.5
ggsave("2b-Epi-MZT-Enzyme-Expression/Output/Fig6-Mcap-Dev-MZT-Expression.pdf", Fig.5, width=21, height=14)
```
