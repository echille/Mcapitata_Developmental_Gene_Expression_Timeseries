...
```{r}
Mcap.DNMT3 <- DNMT3.counts %>% subset(gene_id%in%top_DNMT3) %>% ggplot(aes(x=time_point, y=mean, group=gene_id)) + 
...
```{r}
Mcap.DNMT1 <- DNMT1.counts %>% subset(gene_id%in%top_DNMT1) %>% ggplot(aes(x=time_point, y=mean, group=gene_id)) + 
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +
  geom_line(aes(), lty="dashed", color = "darkgrey") +
  geom_point(size = 1) +
  xlab("Timepoint") +
...
```{r}
Mcap.MBD2 <- MBD2.counts %>% subset(gene_id%in%top_MBD) %>% ggplot(aes(x=time_point, y=mean, group=gene_id)) + 
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.1) +
