...
## Blast
```{r}
blast <- read_tsv("RNAseq_Analyses/annot/Mcap.annot.200806.tab", col_names = FALSE)
...
```{r}
u1 <- read_tsv("RNAseq_Analyses/annot/Mcap_uniprot1.tab", col_names = TRUE)
u1 <- u1[,c(1,4:12)]
...
```{r}
u2a <- read_tsv("RNAseq_Analyses/annot/uniparc-to-uniprot/Mcap_uniprot2.tab", col_names = TRUE)
u2a <- u2a[,c(1,3:11)]
colnames(u2a) <- c("uniparc_entry", "uniprotkb_entry", "status", "protein_names", "gene_names", "organism", "length", "gene_ontology", "go_ids", "ko")

u2b <- read_tsv("RNAseq_Analyses/annot/uniparc-to-uniprot/uniparc-to-uniprot.tab", col_names = TRUE)
...
```{r}
u3 <- read_tsv("RNAseq_Analyses/annot/EMBL-CDS-to-uniprot.tab", col_names = TRUE)
u3 <- u3[,c(1,3:11)]
...
```{r}
u4 <- read_tsv("RNAseq_Analyses/annot/EMBL-prot-to-uniprot.tab", col_names = TRUE)
u4 <- u4[,c(1,3:11)]
...
```{r}
B2G_results <- read_tsv("RNAseq_Analyses/annot/Mcap_blast_200806_GO_200811_Interpro_200824.txt", col_names = TRUE)
...
#write_tsv(Mcap_annot, "RNAseq_Analyses/annot/200824_Mcap_Blast_GO_KO.tsv")
```
...
Load old annotation
```{r}
old_annot <- read.csv("RNAseq_Analyses/annot/OLD_200306_Mcap_annotations.csv", sep=",")
head(old_annot)
nrow(old_annot)
```
